...
function scored(team) {
    if(team === teams[0].name)      // pink has scorred
        teams[0].score++;
    else if(team === teams[1].name) // teal has scorred
        teams[1].score++;
    
    if(teams[0].score === 7 || teams[1].score === 7){ // score will reset after 5
        teams[0].score = 0;
        teams[1].score = 0;
    }
...
function ballEdges(ball) {
    ...
    // check if the ball is outside of the goal
